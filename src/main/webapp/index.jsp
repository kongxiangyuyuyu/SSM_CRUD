						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="email_update_input"
									name="email" placeholder="email@kxy.com"> <span
	...
	<script type="text/javascript">
			...
			$.each(emps, function(index, item) {
				...
				/* 
					...
					<button class="btn btn-danger btn-sm">
								<span class="glyphicon glyphicon-remove-sign "aria-hidden="true"></span>&nbsp;删除
					</button>
				 */
				var editBtn = $("<button></button>").append().addClass(
						"btn btn-primary btn-sm edit_Btn").append(
						$("<span></span>").addClass(
								"glyphicon glyphicon-plus-sign")).append("编辑");
				//为编辑按钮添加自定义的属性，来表示当前员工的id
				editBtn.attr("edit-id",item.empId);
				 
				var deleteBtn = $("<button></button>").append().addClass(
				...
				$("<tr></tr>").append(checkBoxTd).append(empIdTd).append(empNameTd).append(
						genderTd).append(emailTd).append(deptName).append(btn)
		...
		function build_page_info(result) {
			...
			$("#page_info_area").append(
					"当前第" + result.extend.pageInfo.pageNum + "页，总共有"
		...
		function build_page_nav(result) {
			...
			if (result.extend.pageInfo.hasNextPage == false) {
				...
				lastPageLi.click(function() {
					...
					to_pageNum(result.extend.pageInfo.pages);
		...
		$("#emp_save_btn").click(function() {
			...
			//1	先对要提交的数据进行校验，然后再保存
			if (!validate_add_form()) {
				return false;
			} 

			//2	发送ajax之前判断用户名校验是否成功
